...
def test_ai_score_settings_can_be_overridden() -> None:
    settings = load_settings(
        {
            "OFFERFORGE_AI_SCORE_PROVIDER": "openai",
            "OPENAI_API_KEY": " test-key ",
            "OFFERFORGE_OPENAI_MODEL": "gpt-test",
            "OFFERFORGE_AI_SCORE_TIMEOUT_SECONDS": "7",
        }
    )

    assert settings.ai_score_provider == "openai"
    assert settings.openai_api_key == "test-key"
    assert settings.openai_model == "gpt-test"
    assert settings.ai_score_timeout_seconds == 7
