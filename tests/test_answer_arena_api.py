...
from app.services.answer_arena import OpenAIAnswerScoringProvider
    ...
    return json.dumps(
        {
            ...
            "problems": ["案例还可以更具体"],
            "risk_expressions": [],
            "suggestions": ["补充一个项目例子"],
            "optimized_answer_30s": "我会先直接回答，再补充两个要点和一个项目例子，最后强调质量闭环。",
            "memory_labels": ["结论先行", "两点一例", "质量闭环"],
        },
        ensure_ascii=False,
    )


async def test_score_api_ai_mode_without_api_key_returns_clear_error(
    ...
    response = await client.post(
        ...
        json={
            "card_id": card.id,
            "user_answer": "我的理解是需要先正面回答问题，然后补充两个结构化要点，再结合项目接口权限或回归例子，最后总结岗位匹配和质量风险。",
            "mode": "ai",
        },
    )

    assert response.status_code == 503
    assert "AI评分未配置" in response.json()["detail"]


async def test_score_api_ai_mode_missing_card_returns_404(
...
async def test_score_api_ai_mode_rejects_short_answer(
    monkeypatch: pytest.MonkeyPatch,
    client: httpx.AsyncClient,
    db_session: Session,
) -> None:
    ...
    response = await client.post(
        "/api/v1/answer-arena/score",
        json={"card_id": card.id, "user_answer": "太短", "mode": "ai"},
    )

    assert response.status_code == 422


async def test_score_api_ai_mode_mock_success_does_not_write_attempt_or_card(
    monkeypatch: pytest.MonkeyPatch,
    client: httpx.AsyncClient,
    db_session: Session,
) -> None:
    ...
    get_settings.cache_clear()
    monkeypatch.setattr(
        OpenAIAnswerScoringProvider,
        "_request_score",
        lambda self, *, card, user_answer: valid_ai_json(),
    )
    card = create_card(db_session)
    before = {
        "mastery_level": card.mastery_level,
...
async def test_score_api_ai_mode_invalid_provider_payload_returns_clear_error(
    monkeypatch: pytest.MonkeyPatch,
    client: httpx.AsyncClient,
    db_session: Session,
) -> None:
    ...
    monkeypatch.setattr(
        OpenAIAnswerScoringProvider,
        "_request_score",
        lambda self, *, card, user_answer: '{"total_score": 82}',
    )
    card = create_card(db_session)
    ...
    assert response.status_code == 503
    assert "AI评分服务暂不可用" in response.json()["detail"]


async def test_score_api_ai_mode_timeout_returns_clear_error(
    monkeypatch: pytest.MonkeyPatch,
    client: httpx.AsyncClient,
    db_session: Session,
) -> None:
    ...
    def raise_timeout(self, *, card, user_answer):
        raise TimeoutError("timeout")

    monkeypatch.setattr(OpenAIAnswerScoringProvider, "_request_score", raise_timeout)
    card = create_card(db_session)
    ...
    assert "AI评分超时" in response.json()["detail"]
