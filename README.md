...
Answer Arena V0.3: AI scoring also returns example-first coaching fields, including missing points, a complete answer, concrete examples, 60s/30s spoken answers, follow-up questions, and the next practice step.
...
`GET /api/v1/cards/sources` shows card counts by `source_reference`, including total, active, and inactive counts. `PATCH /api/v1/cards/sources/{source_reference}/active` can enable or disable all cards from one source without deleting cards or practice attempts. `DELETE /api/v1/cards/sources/{source_reference}` cleans up a bad or partial seed import only when that source has no practice attempts; otherwise it returns 409 to protect history. Different `source_reference` values may contain the same `title`/`category`; duplicate checks apply within one `source_reference`. `GET /api/v1/reviews/today` only returns active cards, while done-today remains a same-day history view. To switch online use toward v4 manually: import `data_seed/cards_seed_week1_interview_v4.json` through `POST /api/v1/cards/bulk`, confirm v3/v4 counts with the sources API, then set `interview-week1-v3` inactive and verify `/app`. If a v4 import is partial and unpracticed, delete that v4 source first, then bulk import the complete seed again.
